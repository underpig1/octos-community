...
float mapScene(vec3 p) {
    ...
    vec3 q = p;
    q.z += iTime * .5 * iSpeed; //move forward
    q = mod(q, 1.) - .5; //repeating space

    float b1 = sdBox(q, vec3(.1, .1, .3)); //long box
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(iColorScheme == 1) { //custom colors
        vec3 col1 = iColor1;
        vec3 col2 = iColor2;
        col = mix(col1, col2, distTravelled*.1 + float(i) * .015); //color w depth + edges
    }
    else
